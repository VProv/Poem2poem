...
def create_word_batch(data, batch_size, lines_per_doc, nlines_per_batch, pad_symbol, end_symbol, unk_symbol,
                      shuffle_data):
    """
    Creates word batch
    :param data:
...
def print_rm_attention(b, batch_size, num_context, attentions, pad_id, idxchar):
    ...
    for exid in range(batch_size)[-10:]:
        ...
        for ci, c in enumerate(b[0][(exid * num_context + batch_size):((exid + 1) * num_context + batch_size)]):
            print(
                ("\t", ("%.2f" % attentions[exid][ci]), "=", "".join([idxchar[item] for item in c if item != pad_id])))
