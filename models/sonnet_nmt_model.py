...
class SonnetModel(object):
    def __init__(self, is_training, batch_size, word_type_size, char_type_size,
                 space_id, pad_id, cf, name, inp_voc, out_voc):
    ...
    def get_last_hidden(self, h, xlen):
        ...
        gather_ids = tf.concat(
            axis=1,
            values=[tf.expand_dims(ids, 1),
                    tf.expand_dims(xlen - 1, 1)])
        return tf.gather_nd(h, gather_ids)

    def gated_layer(self, s, h, sdim, hdim):

        update_w = tf.get_variable("update_w", [sdim + hdim, hdim])
        update_b = tf.get_variable(
            "update_b", [hdim], initializer=tf.constant_initializer(1.0))
        reset_w = tf.get_variable("reset_w", [sdim + hdim, hdim])
        reset_b = tf.get_variable(
            "reset_b", [hdim], initializer=tf.constant_initializer(1.0))
        c_w = tf.get_variable("c_w", [sdim + hdim, hdim])
        c_b = tf.get_variable(
            "c_b", [hdim], initializer=tf.constant_initializer())

        z = tf.sigmoid(
            tf.matmul(tf.concat(axis=1, values=[s, h]), update_w) + update_b)
        r = tf.sigmoid(
            tf.matmul(tf.concat(axis=1, values=[s, h]), reset_w) + reset_b)
    ...
    def selective_encoding(self, h, s, hdim):
        ...
        attend_b = tf.get_variable(
            "attend_b", [hdim], initializer=tf.constant_initializer())

        g = tf.sigmoid(
            tf.matmul(tf.concat(axis=1, values=[h_, s_]), attend_w) + attend_b)
    ...
    def init_lm(self, is_training, batch_size, word_type_size):
        ...
        self.emb_inp = tf.keras.layers.Embedding(len(self.inp_voc), self.emb_size)
        self.emb_out = tf.keras.layers.Embedding(len(self.out_voc), self.emb_size)

        # encoder lstm cell
        lm_enc_cell = tf.nn.rnn_cell.LSTMCell(cf.lm_enc_dim, forget_bias=1.0, state_is_tuple=False)
        if is_training and cf.keep_prob < 1.0:
            lm_enc_cell = tf.nn.rnn_cell.DropoutWrapper(
                lm_enc_cell, output_keep_prob=cf.keep_prob)
        ...
        # encoder #
        ###########
        # embedding lookup history
        hist_inputs = self.emb_inp(self.lm_hist)
        if is_training and cf.keep_prob < 1.0:
            hist_inputs = tf.nn.dropout(hist_inputs, cf.keep_prob)

        # embedding lookup
        word_inputs = self.emb_inp(self.lm_x)
        if is_training and cf.keep_prob < 1.0:
        ...
        if is_training and cf.keep_prob < 1.0:
            lm_dec_cell = tf.nn.rnn_cell.DropoutWrapper(
                lm_dec_cell, output_keep_prob=cf.keep_prob)

        self.lm_dec_cell = tf.nn.rnn_cell.MultiRNNCell([lm_dec_cell] * cf.lm_dec_layer_size)

        # initial states
        self.lm_initial_state = self.lm_dec_cell.zero_state(
            batch_size, tf.float32)
        state = self.lm_initial_state

        # pad symbol vocab ID = 0; create mask = 1.0 where vocab ID > 0 else 0.0
        lm_mask = tf.cast(
            tf.greater(self.lm_x, tf.zeros(
                tf.shape(self.lm_x), dtype=tf.int32)),
            dtype=tf.float32)
        ...
        hist_outputs, _ = tf.nn.bidirectional_dynamic_rnn(
            cell_fw=lm_enc_cell,
            cell_bw=lm_enc_cell,
            inputs=hist_inputs,
            sequence_length=self.lm_hlen,
            dtype=tf.float32)

        # full history encoding
        full_encoding = tf.concat(
            axis=1,
            values=[hist_outputs[0][:, -1, :], hist_outputs[1][:, 0, :]])
        ...
        with tf.variable_scope("selective_encoding"):
            hist_outputs = self.selective_encoding(hist_outputs, full_encoding,
                                                   cf.lm_enc_dim * 2)

        # attention (concat)
        with tf.variable_scope("lm_attention"):
            attend_w = tf.get_variable(
                "attend_w",
                [cf.lm_enc_dim * 2 + cf.lm_dec_dim, cf.lm_attend_dim])
            attend_b = tf.get_variable(
                "attend_b", [cf.lm_attend_dim],
        ...
        dec_outputs_t = tf.reshape(
            tf.tile(dec_outputs, [1, 1, enc_steps]),
            [batch_size, -1, cf.lm_dec_dim])

        # compute e
        hist_dec_concat = tf.concat(
            axis=1,
            values=[
                tf.reshape(hist_outputs_t, [-1, cf.lm_enc_dim * 2]),
                tf.reshape(dec_outputs_t, [-1, cf.lm_dec_dim])
            ])
        e = tf.matmul(
            tf.tanh(tf.matmul(hist_dec_concat, attend_w) + attend_b), attend_v)
        ...
        context = tf.reduce_sum(
            tf.reshape(alpha * hist_outputs_t,
                       [batch_size, dec_steps, enc_steps, -1]), 2)

        # save attention weights
        self.lm_attentions = tf.reshape(alpha,
                                        [batch_size, dec_steps, enc_steps])
        ...
        with tf.variable_scope("gated_unit"):
            hidden = self.gated_layer(context, dec_outputs, cf.lm_enc_dim * 2,
                                      cf.lm_dec_dim)
            # hidden = dec_outputs

        # output embeddings
        lm_output_proj = tf.get_variable(
            "lm_output_proj", [cf.word_embedding_dim, cf.lm_dec_dim])
        lm_softmax_b = tf.get_variable(
            "lm_softmax_b", [word_type_size],
            initializer=tf.constant_initializer())
        lm_softmax_w = tf.transpose(
            tf.tanh(tf.matmul(self.word_embedding, lm_output_proj)))
        ...
        lm_crossent = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=lm_logits, labels=tf.reshape(self.lm_y, [-1]))
        ...
        lm_grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.lm_cost, lm_tvars), cf.max_grad_norm)
        self.lm_train_op = tf.train.AdagradOptimizer(
            cf.lm_learning_rate).apply_gradients(zip(lm_grads, lm_tvars))
    ...
    def generate(self,
        ...
        while total_words < max_words and total_lines < max_lines:
            ...
            for _ in range(sent_sample):
                one_sent, one_state, one_pl = self.sample_sent(
                    sess, state, x, hist, hlen, xchar, xchar_len,
                    avoid_symbols, stopwords, temp_min, temp_max,
                    unk_symbol_id, pad_symbol_id, end_symbol_id, space_id,
                    idxchar, charxid, idxword, wordxchar, rm_target_pos,
                    rm_target_neg, rm_threshold, last_words, max_words)
                if one_sent != None:
            ...
            if len(all_sent) == 0:
                ...
                probs = probs.astype(
                    np.float64)  # convert to float64 for higher precision
            ...
            if verbose:
                sys.stdout.write(
                    "  Number of generated lines = %d/4\r" % (total_lines))
                sys.stdout.flush()
